...
ones), or whenever a match is logged in season_log.json (so the played
fixture drops out of the schedule picker) or a new fixture list is added.
"""
import csv
import json
import re
...
def load_fixtures(path):
    """Every not-yet-played fixture from season_log.json's "fixtures" array,
    across all competitions -- the schedule picker on submit.html filters
    these client-side by whichever competition is currently selected."""
...
def main():
    ...
    fixtures = load_fixtures('season_log.json')
    ...
    out.append('// or after a match/fixture is added to season_log.json.')
    ...
    out.append('var SCHEDULED_FIXTURES = ' + json.dumps(fixtures, ensure_ascii=False, indent=2) + ';')
    ...
    print(f'Wrote docs/assets/submit_data.js — {len(senior_names)} senior, '
          f'{len(academy_names)} academy, {len(fixtures)} remaining fixtures.')
