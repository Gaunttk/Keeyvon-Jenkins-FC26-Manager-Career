...
| `scripts/squad_analysis.py` | **Read-only** squad performance + depth audit — the analysis that used to be redone by hand every time. Aggregates `season_log.json`, `wrexham_squad.csv` and `youth_academy.csv` into: per-player rating **relative to their own team's average in the same match** (which cancels out opposition strength — a 7.0 in a 0-4 defeat outranks a 7.0 in a 4-1 win), usage-vs-performance flags, goalkeeper splits, pairing interaction effects, and a position-by-position depth ladder scored against the squad standard on two separate axes (**NOW** = current OVR, **CEILING** = scouting tier). Writes nothing — safe to run any time. `--json` for machine-readable output, `--section <name>` to limit it. Uses `lineup` minutes when a match carries them and falls back to counting appearances when it doesn't, stating which mode it ran in. Run it after any match session, and **before** writing any analysis piece — never recompute these numbers by hand |
...
### Session Submit Form — no longer the primary input path

**As of 2027-11 the user has moved back to sending screenshots directly**, having found it faster than filling the form. Don't prompt for form output or assume a session's input will arrive as a `# Session Input Template` block. The form still works and its output is still valid if one does turn up — everything below still applies — but the default assumption for a match session is now: a starting XI in formation, a post-match player ratings screen, a match recap/timeline, a team stats screen, and a league table when it moved. See the `lineup` field in "Season Log Schema" for what the first two of those unlock. `scripts/sync_submit_roster.py` is consequently maintenance for a page that isn't being used — keep running it (it's cheap and keeps the form usable) but it is no longer on the critical path.
...
- `season_log.json`'s `formation` + `lineup` — whenever a starting-XI-in-formation screenshot and a match recap/timeline both came in. This is what makes positions and minutes real rather than inferred from the CSV's position list; see "Season Log Schema" below
- `python3 scripts/squad_analysis.py` — read-only sanity check on what the match just changed (usage flags, depth ladder). Writes nothing; run it before any analysis or Media Centre piece that quotes a performance number
...
**Minutes played and positions ARE now tracked**, via the `lineup` field (see "Season Log Schema" below) — this reverses an earlier note here saying they weren't capturable, which was written when the workflow ran through `docs/submit.html`. The user now sends screenshots directly, including a **starting XI in formation** and a **match recap/timeline**. Between them, every player's position and minutes are derivable without estimating: a starter played to 90 (plus stoppage/extra time, which is not modelled) unless the recap shows him substituted off or sent off; a substitute played from the minute he came on. Capture this whenever both screenshots are present, and leave `lineup` off entirely when they aren't — a partial or guessed lineup is worse than none, because `scripts/squad_analysis.py` treats the field's presence as a promise that the minutes are real.
...
**`formation` and `lineup` (optional, on `matches` entries).** Where each player actually played and for how long — the single highest-value field on a match entry, and the one whose absence caused a real analytical error (Leo Sauer was assumed to be a left winger from his CSV position list when he is picked at right wing; the published squad audit had both flanks' verdicts wrong until the user corrected it). Shape:
```json
"formation": "4-2-3-1",
"lineup": {
  "R. Ngumoha": { "pos": "LW", "start": 0,  "off": 68 },
  "L. Sauer":   { "pos": "RW", "start": 0,  "off": null },
  "A. Minda":   { "pos": "RW", "start": 68, "off": null }
}
```
Keys use the same `"F. Lastname"` abbreviation as `player_ratings` and `transfers`. `pos` is the slot he actually occupied in the formation, **not** his CSV position list. `start` is the minute he came on (`0` for a starter); `off` is the minute he left (`null` if he finished the match, including a starter who played the full 90). Minutes are derived from these two numbers — never store a separate minutes field, it only drifts. A sending-off is an `off` value like any substitution.

Sources: the **starting XI in formation** screenshot gives `pos` for the eleven; the **match recap/timeline** gives substitutions and cards with minute markers, which fills in `start`/`off` for everyone. Include a player only when both are genuinely readable — an omitted player is handled correctly (he falls back to appearance-only), whereas a guessed minute silently corrupts every per-90 figure downstream. **Extra time is not modelled**: for a match that went to extra time, cap `off` at 90 and note it in the milestone rather than writing 120.

`scripts/squad_analysis.py` uses this for per-90 output, position-specific ratings, and minutes-weighted partnership analysis; without it, that script falls back to counting appearances and says so in its header.
